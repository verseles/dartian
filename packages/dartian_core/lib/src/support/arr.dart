...
class Arr {
  ...
  static List<List> crossJoin(List<Iterable> arrays) {
  ...
  /// Determine if the array has any of the given keys.
  ...
  /// Determine if the array has all of the given keys.
  ...
  static String join(Iterable array, String glue, [String? finalGlue]) {
    ...
    return list.join(glue) + finalGlue + last.toString();
  }
  ...
  static Map sortRecursive(Map array, [bool descending = false]) {
    final keys = array.keys.toList();
    keys.sort();
    if (descending) {
      // keys.sort((a, b) => b.compareTo(a)); // keys might not be comparable?
      // Assuming keys are strings or comparable
      try {
        keys.sort((a, b) => (b as Comparable).compareTo(a));
      } catch (e) {
        // ignore
      }
    }
    ...
    for (final key in keys) {
      var value = array[key];
      if (value is Map) {
        value = sortRecursive(value, descending);
      } else if (value is List) {
        // Sort list? Laravel does sort sub-arrays.
        // For lists, we might not sort unless elements are maps?
        // Laravel's sortRecursive also sorts numeric arrays.
        // We will skip list sorting for now to avoid complexity with mixed types.
      }
      result[key] = value;
    }
  ...
  /// Conditionally compile classes from an array into a CSS class string.
  static String toCssClasses(dynamic array) {
    final list = <String>[];

    if (array is String) return array;
    if (array is Iterable) {
      for (final item in array) {
        if (item is String && item.isNotEmpty) {
          list.add(item);
        } else if (item is Map || item is Iterable) {
          final sub = toCssClasses(item);
          if (sub.isNotEmpty) list.add(sub);
        }
      }
    } else if (array is Map) {
      array.forEach((key, value) {
        if (value == true) list.add(key.toString());
      });
    }
  ...
  static Map undot(Map array) {
    final result = {};

    array.forEach((key, value) {
      set(result, key.toString(), value);
    });
  ...
  /// Filter the array using the given callback or key/value pair.
  static List<T> where<T>(Iterable<T> array, dynamic key, [dynamic value]) {
    if (key is Function) {
      return array.where((item) => (key as Function)(item) as bool).toList();
    }

    if (key is String) {
      return array.where((item) => get(item, key) == value).toList();
    }
  ...
  static List<T> whereNotNull<T>(Iterable<T> array, [String? key]) {
    if (key == null) return array.where((item) => item != null).toList();
    return array.where((item) => get(item, key) != null).toList();
  }
}
