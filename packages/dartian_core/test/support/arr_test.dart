...
void main() {
  group('Arr', () {
    ...
    test('hasAny', () {
      expect(
        Arr.hasAny({'name': 'John', 'age': 20}, ['name', 'email']),
        isTrue,
      );
      expect(
        Arr.hasAny({'name': 'John', 'age': 20}, ['email', 'phone']),
        isFalse,
      );
    });

    test('hasAll', () {
      expect(Arr.hasAll({'name': 'John', 'age': 20}, ['name', 'age']), isTrue);
      expect(
        Arr.hasAll({'name': 'John', 'age': 20}, ['name', 'email']),
        isFalse,
      );
    });
    ...
    test('where with callback', () {
      final array = [1, 2, 3, 4];
      expect(Arr.where(array, (item) => item > 2), [3, 4]);
    });
    ...
    test('query', () {
      expect(Arr.query({'name': 'Taylor', 'age': 30}), 'name=Taylor&age=30');
      expect(
        Arr.query({
          'tags': ['a', 'b'],
        }),
    ...
    test('sortRecursive', () {
      final array = {
        'users': [
          {'name': 'Taylor', 'mail': 'taylor@laravel.com'},
          {'name': 'Abigail', 'mail': 'abigail@laravel.com'},
        ],
        'meta': {'key': 'value', 'sort': 'asc'},
      };
      final sorted = Arr.sortRecursive(array);
      expect(sorted.keys.toList(), ['meta', 'users']);
      expect((sorted['meta'] as Map).keys.toList(), ['key', 'sort']);
    });

    test('toCssClasses', () {
      expect(Arr.toCssClasses({'p-4': true, 'font-bold': false}), 'p-4');
      expect(
        Arr.toCssClasses([
          'p-4',
          {'font-bold': true},
    ...
    test('undot', () {
      expect(Arr.undot({'user.name': 'Taylor', 'user.age': 25}), {
    ...
    test('where', () {
      ...
      expect(Arr.where(array, 'price', 100), [
        {'product': 'Chair', 'price': 100},
      ]);
    });

    test('whereNotNull', () {
      final array = [1, null, 2];
      expect(Arr.whereNotNull(array), [1, 2]);
    });
  });
}
